...
pub fn route_add_with_if(dst: u32, mask: u32, gw: u32, if_index: u32) -> Result<()> {
    ...
    match ret {
        ...
        ERROR_INVALID_PARAMETER => Err(anyhow!("route add invalid parameter")),
        ERROR_NOT_SUPPORTED => Err(anyhow!("route add not supported")),
        ERROR_ACCESS_DENIED => Err(anyhow!("route add access denied")),
        _ => Err(anyhow!("route add error unknown")),
    }
}
