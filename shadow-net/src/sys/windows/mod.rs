...
pub async fn create_inbound_udp_socket(addr: &SocketAddr) -> io::Result<UdpSocket> {
    ...
    let socket = if !set_dual_stack {
        ...
        if let Err(err) = socket.set_only_v6(false) {
            warn!(
                "failed to set IPV6_V6ONLY: false for listener, error: {}",
                err
            );
        ...
        match socket.bind(&saddr) {
            ...
            Err(ref err) if err.kind() == ErrorKind::AddrInUse => {
                ...
                if let Err(err) = socket.set_only_v6(true) {
                    warn!(
                        "failed to set IPV6_V6ONLY: true for listener, error: {}",
                        err
                    );
...
#[inline(always)]
pub async fn create_outbound_udp_socket(
    af: AddrFamily,
    opts: &ConnectOpts,
) -> io::Result<UdpSocket> {
...
pub fn set_common_sockopt_after_connect<S: AsRawSocket>(
    stream: &S,
    opts: &ConnectOpts,
) -> io::Result<()> {
